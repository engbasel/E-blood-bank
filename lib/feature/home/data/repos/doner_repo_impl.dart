...
import 'package:blood_bank/core/mapper/donor_mapper.dart';
...
class DonorRepoImpl implements DonorRepo {
  ...
  Future<Either<Failures, void>> addRequest(
    ...
    try {
      await _donorRemoteDataSource
          .addDonorRequest(DonorModel.fromEntity(addRequestInputEntity));
  ...
  Stream<List<DonorRequestEntity>> getAllDonorRequests() {
    return _donorRemoteDataSource.getAllDonorRequests()
        .map((models) => models.map(DonorMapper.toEntity).toList());
  }
}
